...
def Hit_ship(s_x, s_y):
    ...
    if isTurn == False:
        if s_x == sampleShip_x1 and s_y == sampleShip_y1:
            ...
            player1_play[s_x][s_y].configure(text="N", fg="black", bg="white", state="disable")
    else:
        if s_x == sampleShip_x6 and s_y == sampleShip_y6:
            ...
            player2_play[s_x][s_y].configure(text="N", fg="black", bg="white", state="disable")
...
def create_label(lb, txt, x, y):
    if lb == "label":
        ...
        label = Button(display, text=txt, width=2, height=1, font=label_font, fg="black", bg="Orange",
                       command=partial(Hit_ship, x, y))
    return label
...
# creat title for each board, show player1 and player2
def showlabel():
    # title
...
# create ships which can show on the board
def demo():
